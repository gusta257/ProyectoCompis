...
from Proyecto3 import CodigoTarget
...
class DecafPrueba(DecafListener):
    ...
    def enterLocation(self, ctx:DecafParser.LocationContext):
        ...
        if(esArray):
            ...
            total = self.revisarHijo(ctx.location(), tipo)
    ...
    def revisarHijo(self, location, parent_type):
        ...
        if esArray:
            ...
            num = self.revisarHijo(location.location(), child_type)
    ...
    def exitStatementWHILE(self, ctx:DecafParser.StatementWHILEContext):
        ...
        || gen ( goto inicio)
        '''
        code = [S['next']]+['\t'+ x for x in B['codigo']]+[B['true']]+['\t'+ x for x in S['codigo']]+['\t'+ 'GOTO '+S['next']]+[B['false']] #+ [B['true']] + S['codigo'] + [B['false']]
        #print("*"*20)
    ...
    def exitStatementIF(self, ctx:DecafParser.StatementIFContext):
        ...
        if(len(ctx.children) > 5):
            ...
            code = B['codigo']+[B['true']]+['\t'+ x for x in S1['codigo']]+['\tGOTO '+ S1['next']]+[B['false']] +['\t'+ x for x in S2['codigo']]+[S1['next']]
            #print("*"*20)
            ##print(code)
            #print("CODIGO INTERMEDIO")
            for i in code:
    ...
    def enterStatementIF(self, ctx:DecafParser.StatementIFContext):
        ...
        if(len(ctx.children) > 5):
            ...
            #print("MANDANDO EXPR",self.nodeCodes[expr])
            self.nodeCodes[blockIF] = {
                'next': b_next
            }
            #print("MANDANDO blockIF",self.nodeCodes[blockIF])
            self.nodeCodes[blockELSE] = {
                'next': b_next
            }
            #print("MANDANDO blockELSE",self.nodeCodes[blockELSE])
    ...
    def exitMethodDeclaration(self, ctx:DecafParser.MethodDeclarationContext):
        ...
        code = ['DEF '+nombre+":"]+['\t'+ x for x in codigo['codigo']]+['EXIT DEF '+nombre]+['\n']
        #print("*"*20)
    ...
    def exitStatementRETURN(self, ctx:DecafParser.StatementRETURNContext):
        ...
        code = self.nodeCodes[expr]['codigo']+ ['RETURN ' + self.nodeCodes[expr]['dir']]
